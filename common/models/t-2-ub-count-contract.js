...
module.exports = function(T2ubcountcontract) {

    T2ubcountcontract.processChild = function(req, filter, cb) {
      this.process(req, filter, function(err, message) {
        if (err) {
          return cb(err, null);
      }
        var jsonData = {
          "UBAR_DATE":message[0].DATA_LOAD_DESC,
          "data": [
          {
            "label": "1-30",
            "value": "BUCK_01",
  ...
  T2ubcountcontract.remoteMethod('processChild', {
      http: {path: '/', verb: 'get', status: 200},
      accepts: [{arg: 'data', type: 'object',  http: {source: 'req'} },
                  {arg: 'filter', type: 'object'}],
      returns: {arg: 't2UBCountContract', type: 'object'}
  });
