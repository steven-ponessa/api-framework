...
module.exports = function(T2compdmdelusgtrend) {
  T2compdmdelusgtrend.processChild = function(req, filter, cb) {
      this.process(req, filter, function(err, message){
        ...
        if (!message.length <= 0) {
          ...
          switch (req.query['metric']) { 
            ...
            case "NCOM":
              metColor = "#00539A";
              metLabel = "Delivery Not Compliant";
            break;
            case "APR":
              metColor = "#A1D800";
  ...
  T2compdmdelusgtrend.remoteMethod('processChild', {
    http: {path: '/', verb: 'get', status: 200},
    accepts: [{arg: 'data', type: 'object',  http: {source: 'req'} },
              {arg: 'filter', type: 'object'}],
              returns: {arg: 't2CompDMDelUsgTrend', type: 'object'}
  });
