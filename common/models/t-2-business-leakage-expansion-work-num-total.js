...
module.exports = function(T2businessleakageexpansionworknumtotal) {
  T2businessleakageexpansionworknumtotal.processChild = function(req, filter, cb) {
      ...
      try {
            ...
            jsonData.data[0].percent = calcs.calcPercent(Number(message[0].ITD_TO_PC_LKG), Number(message[0].ITD_ACT_REV));
            jsonData.data[0].status = calcs.getStatusNoColor(Number(message[0].PRI_ITD_TO_PC_LKG), Number(message[0].PRI_ITD_ACT_REV), Number(message[0].ITD_TO_PC_LKG), Number(message[0].ITD_ACT_REV));
            ...
            jsonData.data[2].percent = calcs.calcPercent(Number(message[0].FC_TO_PC_LKG), Number(message[0].PC_PLAN_REV));
            jsonData.data[2].status = calcs.getStatusNoColor(Number(message[0].PRI_FC_TO_PC_LKG), Number(message[0].PRI_PC_PLAN_REV), Number(message[0].FC_TO_PC_LKG), Number(message[0].PC_PLAN_REV));
  ...
  T2businessleakageexpansionworknumtotal.remoteMethod('processChild', {
      http: {path: '/', verb: 'get', status: 200},
      accepts: [{arg: 'data', type: 'object',  http: {source: 'req'} },
                  {arg: 'filter', type: 'object'}],
      returns: {arg: 't2BusinessLeakageExpansionWorkNumTotal', type: 'object'}
  });
  };
