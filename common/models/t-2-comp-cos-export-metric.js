...
module.exports = function (T2compcosexportmetric) {
	T2compcosexportmetric.processChild = function (req, filter, cb) {
				...
				if (message.length == 0) {
					...
					for (var i = 0; i < message.length; i++) {
						...
						formatedJson.data.push({
							...
							"COMPLETED_BY": message[i].COMPLETED_BY,
							"COMPLETED_DATE": message[i].COMPLETED_DT,
	...
	T2compcosexportmetric.remoteMethod('processChild', {
		http: { path: '/', verb: 'get', status: 200 },
		accepts: [{ arg: 'data', type: 'object', http: { source: 'req' } },
		{ arg: 'filter', type: 'object' }],
		returns: { arg: 't2CompCOSExportMetric', type: 'object' }
	});
};
