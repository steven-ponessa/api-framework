...
module.exports = function(T2CompNonGDPRDetailsContract) {

    T2CompNonGDPRDetailsContract.processChild = function(req, filter, cb) {
          this.process(req, filter, function(err, message){
            ...
            switch (req.query['metric']) {
                case "NON_GDPR_IMPCT":
                    color = "#BE95FF";
                    value = "Non-GDPR Impacted Contracts";
                break;
                case "LANG":
                    if (type == 'NEGATIVE') {
                        color = "#A6C8FF";
                        value = "Contracts Standard Language Not Included";                
                    } else {
                        color = "#A2A9B0";
                        value = "Contracts Standard Language Included"; 
                    }
                break;
                }
                var formatedJson = {};
    ...
    T2CompNonGDPRDetailsContract.remoteMethod('processChild', {
          http: {path: '/', verb: 'get', status: 200},
          accepts: [{arg: 'data', type: 'object',  http: {source: 'req'} },
                      {arg: 'filter', type: 'object'}],
          returns: {arg: 't2CompNonGDPRDetailsContract', type: 'object'}
      });
